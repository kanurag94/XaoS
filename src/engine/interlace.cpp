...
static int initialize(struct filter *f, struct initdata *i)
{
    ...
    f->childimage = i->image = create_image_lines(
        i->image->width, (i->image->height) / 2, 2, lines1, lines2,
        i->image->palette, NULL, FREELINES | PROTECTBUFFERS,
        f->image->pixelwidth, f->image->pixelheight * 2);
    if (i->image == NULL) {
        free(lines1);
        free(lines2);
        return 0;
...
const struct filteraction interlace_filter = {
    "Interlace filter", "interlace", 0,           getinstance,
