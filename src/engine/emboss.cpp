...
#define SSTEP2 (32 * 8 / 256)
    if (datalost(f, i) || i->image->version != f->imageversion) {
        ...
        if (i->image->palette->type & (C256 | GRAYSCALE)) {
            ...
            for (x = 0; x < 256; x++) {
                ...
                if (dist > 255)
                    dist = 255;
                s->table[x] = ((dist >> i->image->palette->info.truec.rprec)
                               << i->image->palette->info.truec.rshift) |
                              ((dist >> i->image->palette->info.truec.gprec)
                               << i->image->palette->info.truec.gshift) |
                              ((dist >> i->image->palette->info.truec.bprec)
    ...
    if (s->ydist < 1)
        s->ydist = 1;
    if (!inherimage(f, i, TOUCHIMAGE, i->image->width + s->xdist,
...
static void emboss32(void *data, struct taskinfo */*task*/, int r1, int r2)
{
    ...
    for (i = r1; i < r2; i++) {
        ...
        while (src < srcend) {
            *dest = table[((int)*src2 - (int)*src) & 511] |
                    ~(f->image->palette->info.truec.allmask);
            src++;
            src2++;
            dest++;
        }
...
static int doit(struct filter *f, int flags, int time1)
{
    ...
    drivercall(*f->image, xth_function(emboss8, f, f->image->height),
